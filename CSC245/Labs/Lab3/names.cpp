...
void BreakDown (string name, string& first, string& mi, string& last)
{
	...
	// post : first, mi, and last contain the individual components
        //        of that name
        int length = name.length() - last.length() - 5;
	first = name.substr(1, length);
	mi = name.substr(length, 1);
	int lastLength = name.length() - first.length() - 5;
	last = name.substr(name.find(","), lastLength);
