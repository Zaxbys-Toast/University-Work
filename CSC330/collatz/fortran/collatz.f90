program collatz
    implicit none
    integer(kind=8) :: min_range, max_range
    integer(kind=8) :: i, length
    integer(kind=8), allocatable :: idx(:), len(:), tmp_arr(:)
    integer(kind=8) :: tmp_idx, tmp_len, j
    ...
    ! Initialize arrays for lengths and indices (top 10)
    allocate(len(10))
    allocate(idx(10))
    len = 0
    idx = 0
...
contains
    ...
    ! Iterative function to find the Collatz sequence length
    function find_length(i) result(res)
        integer(kind=8), intent(in) :: i
        integer(kind=8) :: res
        integer(kind=8) :: n

        n = i
        res = 1  ! Initialize the sequence length

        ! Iterate to find the Collatz sequence length
        do while (n /= 1)
            if (mod(n, 2) == 0) then
                n = n / 2
            else
    ...
    subroutine sort_by_length(len, idx)
        integer(kind=8), dimension(:), intent(inout) :: len
        integer(kind=8), dimension(:), intent(inout) :: idx
        integer(kind=8) :: i, j, tmp_len, tmp_idx
